...
#'}
#'
#' @importFrom phyloseq otu_table sample_data sample_data<-
...
add_rarefaction_metrics <- function(data) {
    ...
    if (inherits(data, "phyloseq")) {
        otu_mat <- phyloseq::otu_table(data, taxa_are_rows = TRUE)
    ...
    if (inherits(data, "phyloseq")) {
        sample_df <- phyloseq::sample_data(data) %>%
            as.matrix() %>%
            as.data.frame() %>%
            tibble::rownames_to_column("sample_id")
        
        sample_df_updated <- dplyr::left_join(sample_df, df_stats, by = "sample_id") %>%
            #dplyr::mutate(read_num = read_num.y) %>%
            #dplyr::select(-read_num.x, -read_num.y) %>%
            as.data.frame() %>%
            tibble::column_to_rownames("sample_id")
        
        phyloseq::sample_data(data) <- sample_df_updated
        return(data)
        
    } else {
        data_out <- data %>%
            tibble::rownames_to_column("sample_id") %>%
            dplyr::left_join(df_stats, by = "sample_id") %>%
            tibble::column_to_rownames("sample_id")

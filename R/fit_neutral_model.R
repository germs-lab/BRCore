...
#' above (i.e. interpreted as deterministically or predictably selected) and
#' below (i.e. interpreted as dispersally limited) the model predictions.
#'
#' @param otu_table A community count matrix (or OTU table) with samples as
#' rows and OTU/ASVs as columns..
#' @param core_set Character vector of core OTU/ASVs IDs (must match column
#' names of `otu_table`).
#' @param abundance_occupancy data.frame (or tibble) with OTU/ASVs names,
#'  occupancy (`otu_occ`),
#'  and mean relative abundance (`otu_rel`) as generated by

...
source(here::here("R/extract_core_parallel.R"))

# Load the GlobalPatterns dataset
data(GlobalPatterns, package = "phyloseq")

# Add grouping variable
sample_data(GlobalPatterns)$TestGroup <- sample(c("A", "B", "C"), 
                                             nsamples(GlobalPatterns), 
                                             replace = TRUE)

# Create datasets of different sizes
datasets <- list(
  small = prune_taxa(names(sort(taxa_sums(GlobalPatterns), decreasing = TRUE)[1:100]), 
                     GlobalPatterns),
  medium = prune_taxa(names(sort(taxa_sums(GlobalPatterns), decreasing = TRUE)[1:500]), 
                      GlobalPatterns),
  large = prune_taxa(names(sort(taxa_sums(GlobalPatterns), decreasing = TRUE)[1:5000]), 
                     GlobalPatterns)
)
...
run_all_benchmarks <- function() {
  ...
  for (dataset_name in names(datasets)) {
    cat("Starting benchmark for", dataset_name, "at", format(Sys.time(), "%Y-%m-%d %H:%M:%S"), "\n")
    results[[dataset_name]] <- run_size_benchmark(dataset_name)
    cat("Completed benchmark for", dataset_name, "at", format(Sys.time(), "%Y-%m-%d %H:%M:%S"), "\n")
  }
...
cat("All benchmarks completed at", format(Sys.time(), "%Y-%m-%d %H:%M:%S"), "\n")
...
# Save results
write.csv(results_summary, here::here("tests/benchmarks/extract_core_benchmark_summary.csv"), row.names = FALSE)
saveRDS(benchmark_results, here::here("extract_core_benchmark_results_by_size.rds"))
...
if (nrow(results_summary) > 0) {
  ...
  p
  # Save plot
  ggsave(here::here("tests/benchmarks/extract_core_benchmark_by_size.png"),
         p,
         width = 10,
         height = 6)
}
